...
#include "core.hpp"
#include "events/systems/event_dispatcher.hpp"
...
namespace fro
{
   ...
   class EntityManager final
   {
      ...
      private:
         ...
         FRO_API FRO_NODISCARD static std::unordered_map<std::type_index, std::unique_ptr<BaseComponentSparseSet>>&
         getBaseComponentSparseSets();
         ...
         FRO_NODISCARD static SparseSet<ComponentType>& getSparseSet()
         {
            ...
            auto& baseComponentSparseSet{
               getBaseComponentSparseSets().emplace(typeid(ComponentType), std::move(temporary)).first->second
            };
