	...
	if (dx == 0) {
		...
		*/
		if (draw_endpoint)
		{
			...
			if (dy > 0) {
				return (vlineRGBA(renderer, x1, static_cast<Sint16>(yy0), static_cast<Sint16>(yy0+dy), r, g, b, a));
			} else {
				return (pixelRGBA(renderer, x1, y1, r, g, b, a));
			}
		}
		...
		*/
		if (draw_endpoint)
		{
			...
			if (dx > 0) {
				return (hlineRGBA(renderer, static_cast<Sint16>(xx0), static_cast<Sint16>(xx0+(xdir*dx)), y1, r, g, b, a));
			} else {
				return (pixelRGBA(renderer, x1, y1, r, g, b, a));
			}
		}
		...
		*/
		...
		while (--dy) {
			...
			*/
			...
			result |= pixelRGBAWeight (renderer, static_cast<Sint16>(xx0), static_cast<Sint16>(yy0), r, g, b, a, 255 - wgt);
			result |= pixelRGBAWeight (renderer, static_cast<Sint16>(x0pxdir), static_cast<Sint16>(yy0), r, g, b, a, wgt);
		}
		...
		*/
		...
		while (--dx) {
			...
			*/
			wgt = (erracc >> intshift) & 255;
			result |= pixelRGBAWeight (renderer, static_cast<Sint16>(xx0), static_cast<Sint16>(yy0), r, g, b, a, 255 - wgt);
			result |= pixelRGBAWeight (renderer, static_cast<Sint16>(xx0), static_cast<Sint16>(y0p1), r, g, b, a, wgt);
		}
	}
	...
	*/
    ...
    while (deltaX <= deltaY)
    {
		  ...
		  if ((scrX != oldX && scrY == oldY) || (scrX != oldX && scrY != oldY)) {
			result |= _drawQuadrants(renderer, x, y, static_cast<Sint16>(scrX), static_cast<Sint16>(scrY), f);
			oldX = scrX;
			oldY = scrY;
		  }
    }
	...
	if (curY > 0) 
	{
		...
		while (curY > 0)
		{
		    ...
		    if ((scrX != oldX && scrY == oldY) || (scrX != oldX && scrY != oldY)) {
				oldY--;
				for (;oldY >= scrY; oldY--) {
					result |= _drawQuadrants(renderer, x, y, static_cast<Sint16>(scrX), static_cast<Sint16>(oldY), f);
					/* prevent overdraw */
					if (f) {
						oldY = scrY - 1;
					}
...
int _pieRGBA(SDL_Renderer * renderer, Sint16 x, Sint16 y, Sint16 rad, Sint16 start, Sint16 end,  Uint8 r, Uint8 g, Uint8 b, Uint8 a, Uint8 filled)
{
	...
	*/
	...
	/* First vertex */
	angle = start_angle;
	vx[1] = x + (Sint16) (dr * cos(angle));
	vy[1] = y + (Sint16) (dr * sin(angle));
	...
	else
	{
		...
		while (angle < end_angle) {
			...
			vx[i] = x + (Sint16) (dr * cos(angle));
			vy[i] = y + (Sint16) (dr * sin(angle));
			i++;
		}
...
void clearPrimitivesPolyIntsBuffer()
{
	delete gfxPrimitivesPolyIntsGlobal;
	gfxPrimitivesPolyIntsGlobal = nullptr;
	...
	for (y = miny; (y <= maxy); y++) {
		...
		*/
		...
		for (i = 0; (i < ints); i += 2) {
			...
			result |= hline(renderer, static_cast<Sint16>(xa), static_cast<Sint16>(xb), static_cast<Sint16>(y));
		}
	}
	...
	*/
	...
	for (y = miny; (y <= maxy); y++) {
		...
		for (i = 0; (i < ints); i += 2) {
			...
			xb = gfxPrimitivesPolyInts[i+1] - 1;
			xb = (xb >> 16) + ((xb & 32768) >> 15);
			result |= _HLineTextured(renderer, static_cast<Sint16>(xa), static_cast<Sint16>(xb), static_cast<Sint16>(y), textureAsTexture, texture->w, texture->h, texture_dx, texture_dy);
		}
	}
...
int thickLineRGBA(SDL_Renderer *renderer, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint8 width, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	if ((x1 == x2) && (y1 == y2)) {
		wh = width / 2;
		return boxRGBA(renderer, static_cast<Sint16>(x1 - wh), static_cast<Sint16>(y1 - wh), x2 + width, y2 + width, r, g, b, a);
	}
